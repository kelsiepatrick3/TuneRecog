// FFT.java
// 2014 working from 2004 Owie code
...
public class FFT
{
   Complex[][] e2the; // e[c][k] is e ^ -i(2 pi k / (2^c) )
                      ...
                      // to wrap, so watch it.
   int cmax; // the largest value of c above for which e2the is defined
   static final double pi=3.14159265358979323846264338327950288419716939937510;

 
   // default constructor, size of 9 (up to 512 point FFTs)
   public FFT()
   {
      cmax = 9;
      fille();
   ...
   // constructor to set table size other than 9
   public FFT( int cmax1 )
   {
      cmax = cmax1; 
      fille();
   }
 
   // fill in the e2the tables.
   // Note: I think k used to wrap, but here it doesn't.
   // i.e., the rows of this table are different lengths.
   public void fille( )
   {
      int klimit = (int) Math.pow(2,cmax); // size of the cth table
      e2the = new Complex[cmax+1][klimit];
      int c; // which sin cos table to fill
      for ( c=0; c<=cmax; c++ )
      {
          //e2the[c] = new Complex[klimit];
          int cycle =  (int) Math.pow(2,c);
          double a = 2 * pi / cycle; // angular step in radians
          double x=0; // angle
...
for ( p=0; p<P; p++ )
{
 ...
 // It does not alter S.
 public Complex[] fftcore( int N, Complex[] S )
 {
     ...
     { System.out.println("Exception e= "+e+" m="+m+" c="+c); }
     return D;
 }
