...
public class FFT
{
   ...
   Complex[][] e2the; // e[c][k] is e ^ -i(2 pi k / (2^c) )
                    ...
                    // to wrap, so watch it.
   int cmax=12; // the largest value of c above for which e2the is defined
 ...
 public int po2( int N )
 {
   ...
   // maxofrange.  compute the maximum magsq from the range fromhere - tohere-1
 ...
 // cpp replaces fft
 public void transform( double realN, Complex[] S, Complex[] D )
 {
     if ( realN > 512 )
     {  
     ...
     if ( (N == po2(N)) && ((double)N==realN) ) 
     {
         ...
         fftcore( N, S, D ); 
         for ( int i=0; i<N; i++ ) { D[i].multiply(5*oneoverN); }
     }
     else
     {
         ...
         for ( int i=0; i<po2N; i++ ) { D[i].multiply(5*oneoverpo2n); }
     }
     //return D;
 }
...
{
   D[p] = new Complex();
   for ( g=0; g<group; g++ ) { D[p].add(S[n]); n++; }
   D[p].multiply(rat);
}
     return D;
 }

   // fftcore.  This is a raw fft.  It takes N points of S and puts
   // into D the spectrum (FFT), of N points. 
   // N must be a power of 2.
   // It does not alter S.
   public void fftcore( int N, Complex[] S, Complex[] D )
   {
     //Complex D[];
     ...
     else // N>1, divide and recurse
     {
         ...
         Complex[] a = new Complex[Nover2];
         fftcore( Nover2, evens, a );
         Complex[] b = new Complex[Nover2];
         fftcore( Nover2, odds, b );
         
         for ( m=0; m<Nover2; m++ )
         {
            if (bug) {System.out.println("FFT.fftcore: N="+N+" m="+m); }
        	 D[m] = new Complex(b[m]);
        	 D[m].multiply( e2the[c][m] );
        	 D[m].add(a[m]);
        	 
        	 D[m+Nover2] = new Complex(b[m]);
        	 D[m+Nover2].multiply( e2the[c][m+Nover2] );
        	 D[m+Nover2].add(a[m]);
...
return maxi;
 ...
 {      
     ...
     Complex[] spect = new Complex[64]; 
     transform( 64, shrunk, spect ); 
...
Complex[] ft = new Complex[64]; // FFT raw (20 rounded up to power of 2 is 32)
...
for ( n=0; n<P; n++ )
{
   C = new Complex (ftvec[n]);
   C.multiply( 1 / ftabs[n].real );
   D[n].real = C.mag();
}
